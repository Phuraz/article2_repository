...
## Contents
- `Final_Hawkes_Bitcoin_data_PyMC_Model_Comparison_Py310.ipynb`: Initial model-fitting experimentation, including code for saving fitted model objects to disk using `az.to_netcdf()`. Most model-fitting blocks are commented out but can be uncommented and rerun where required. The saved model-output files are not included because they exceed GitHub’s file-size limit. Consequently, to reproduce the workflow from scratch, the model-fitting code must first be executed and the resulting outputs saved locally before running `REVIEWS_Refactored_Final_Bitcon_PyMC_Bayes_Analysis.ipynb`.
